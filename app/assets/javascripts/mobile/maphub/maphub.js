...
/*
 * Easy but naive way of getting URL query parameters. From StackOverflow.
 */
...
/**
 * A callback used to initialize a map.
 */
maphub.initialize = function(containerID) {
	var mapID = getQueryString()['mapid'];
	var map = new maphub.Map({
		deleteHiddenTiles: true,
		id: mapID
	});
	map.render(document.getElementById(containerID));
}
...
/*
 * Convert a google.maps.MVCArray<google.maps.LatLng> into a string.
 */
maphub.stringifyPath = function(path) {
	var s = "";
	 ...
	 */
	var latLng = path.getAt(0);
	s = s+latLng.lat()+" "+latLng.lng();

	/*
	 * If there are more than one, append them to the string.
	 */
	if (pathLength > 1) {
